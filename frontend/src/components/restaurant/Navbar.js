...
import { ShoppingCart, Menu, X, Globe, Clock } from 'lucide-react';
...
export default function Navbar() {
  ...
  useEffect(() => {
    const update = () => {
      const { open, closeTime: ct, openTime: ot } = checkIsOpen();
      setIsOpen(open);
      setCloseTime(ct);
      setOpenTime(ot);
    };
  ...
  return (
    <>
      ...
      {/* Closed Banner - az egész oldalon látható a Navbar alatt */}
      {!isOpen && (
        <div className="fixed top-16 md:top-20 left-0 right-0 z-40 bg-red-900/90 backdrop-blur-sm border-b border-red-500/40 py-2 px-6" data-testid="closed-banner">
          <div className="max-w-7xl mx-auto flex items-center justify-center gap-2">
            <Clock size={14} className="text-red-400 shrink-0" />
            <p className="text-sm text-red-300 font-['Source_Sans_3',sans-serif] text-center">
              {closedMessage}
            </p>
          </div>
        </div>
